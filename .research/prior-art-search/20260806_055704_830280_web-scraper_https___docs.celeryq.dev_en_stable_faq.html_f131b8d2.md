...
Answer: Yes.
worker is an implementation of Celery in Python. If the
...
your broker logs (for RabbitMQ that’s /var/log/rabbitmq/rabbit.log on
...
--purge option enabled to purge messages
...
>>> result.get()
This will give you a AsyncResult instance
...
You can enable this using the broker_use_ssl setting.
...
Answer: See the broker_pool_limit setting.
...
Defaults requiretty
If you have this configuration in your /etc/sudoers file then
...
use the Celery.signature() method:
>>> chain(
...
Note
Both current_task, and current_worker_task can be
None.
...
You can use the --disable-prefetch
flag (or set worker_disable_prefetch to True) so that a worker
...
is catch-able with the try block. The AMQP transaction isn’t used
...
periodic tasks should be preferred.
...
You should never stop worker with the KILL signal
(kill -9), unless you’ve tried TERM a few times and waited a few
