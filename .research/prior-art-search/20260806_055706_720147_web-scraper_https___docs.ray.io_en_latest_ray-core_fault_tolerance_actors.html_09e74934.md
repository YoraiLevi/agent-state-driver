...
description: Actors can fail if the actor process dies, or if the owner of the actor dies. The owner of an actor is the worker that originally created the actor by calling ActorClass.remote(). Detached actors d...
sitename: Ray
date: 2026-01-01
---
Actor Fault Tolerance#
...
- True: Ray retries a method on user exception up to- max_task_retriestimes.
- A list of exceptions: Ray retries a method on user exception up to - max_task_retriestimes, only if the method raises an exception from these specific classes.
